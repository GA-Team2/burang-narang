...
function update_check_ok() {
	...
	if(update_frm.mem_pwd.value.length == 0){
		alert("패스워드는 반드시 입력해야 합니다.");
		update_frm.mem_pwd.focus();
		return;
	}
	
	if(update_frm.mem_pwd.value != update_frm.pwd_check.value){
		alert("패스워드가 일치하지 않습니다.");
		update_frm.pwd_check.focus();
		return;
	}
	
	if(update_frm.mem_email.value.length == 0){
		alert("Email을 써주세요.");
